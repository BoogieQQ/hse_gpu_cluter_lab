...
| **Time-slicing** | 4 | `nvidia.com/gpu.shared` | ~159.25 | 100% | 7.34 | 3 рана отработали в параллель, 1 ожидал, выигрыша по времени нет |

**Вывод:** \
В данной задаче прироста выбить не получилось: если бы последовательно запускали 4 рана, то на них бы ушло по $\sim 150 \times 4 = \sim 600s$, когда на выполнение в time-slicing режиме на 4 джобы ушло суммарно $470+157 = \sim 637s$.

**Почему так получилось:** \
Time-slicing не сработал, потому что обучение BERT и так на 99% загружает процессор видеокарты. В результате тяжелые вычисления при ts встали в очередь, а параллельный запуск только затормозил процесс из-за постоянного переключения видеокарты между подами.
